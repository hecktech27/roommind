...
from ..const import MODE_HEATING, MODE_IDLE
...
class ResidualHeatTracker:
    ...
    def get_q_residual(self, area_id: str, system_type: str, previous_mode: str) -> float:
        ...
        heat_dur = (
            self._off_since[area_id]
            - self._on_since.get(area_id, self._off_since[area_id])
        ) / 60.0
        last_pf = self._off_power.get(area_id, 1.0)
        return compute_residual_heat(elapsed, system_type, last_pf, heat_dur)

    def update(self, area_id: str, mode: str, power_fraction: float, previous_mode: str, q_residual: float = 0.0) -> None:
